...
def test_ECG5000_download_file_exists(mocker: pytest.Mocker, ds: pd.DataFrame) -> None:
...
def test_ECG5000_download_file_not_exist(
    mocker: pytest.Mocker, ds: pd.DataFrame
) -> None:
    exists_mock = mocker.patch("os.path.exists")
    makedirs_mock = mocker.patch("os.makedirs")
    requests_get_mock = mocker.patch("requests.get")
...
def test_ECG5000_download_force_file_exists(
    mocker: pytest.Mocker, ds: pd.DataFrame
) -> None:
    exists_mock = mocker.patch("os.path.exists")
    makedirs_mock = mocker.patch("os.makedirs")
...
def test_ECG5000_download_force_file_not_exist(
    mocker: pytest.Mocker, ds: pd.DataFrame
) -> None:
    exists_mock = mocker.patch("os.path.exists")
    makedirs_mock = mocker.patch("os.makedirs")
